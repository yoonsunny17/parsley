...
function SearchStudyRooms() {
  ...
  return (
    <div>
      <div className="text-font1 font-bold text-2xl mt-9">
        '파슬리' 검색결과
      </div>
      ...
      <div className="px-6 pt-4 md:px-10 w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-5 md:gap-x-5">
      ...
      <div className="px-6 pt-4 pb-8 md:px-10 w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-5 md:gap-x-5">
        {tabContArr[activeIndex].studyRoomList.map((info, idx) => {
          return <StudyRoomItem info={info} key={idx} />;
        })}
      </div>
