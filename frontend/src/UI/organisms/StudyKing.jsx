...
function StudyKing() {
  ...
  const { data: getNongbuRankingsByRoomId } = useGetNongbuRankingByRoomIdQuery(
    params.id,
    {
      refetchOnMountOrArgChange: true,
    }
        ...
        {activeIndex === 0 ? (
          <div className="overflow-x-auto">
            <table className="table w-full">
              ...
              <tbody>
                {getGongbuRankingsByRoomId?.topRank.map((rank, idx) => {
                  return (
                    <tr
                      className={`text-sm ${
                        user.name === rank.name ? "active" : ""
                      }`}
                    >
                      <td>
                        {idx === 0 ? (
                          <BiMedal
                            className={`bx bx-medal text-gold text-3xl`}
                          />
                        ) : idx === 1 ? (
                          <BiMedal
                            className={`bx bx-medal text-silver text-3xl`}
                          />
                        ) : idx === 2 ? (
                          <BiMedal
                            className={`bx bx-medal text-bronze text-3xl`}
                          />
                        ) : (
                          idx + 1
                        )}
          ...
          <div className="overflow-x-auto">
            <table className="table w-full">
              ...
              <tbody>
                {getNongbuRankingsByRoomId?.topRank.map((rank, idx) => {
                  return (
                    <tr
                      className={`text-sm ${
                        user.name === rank.name ? "active" : ""
