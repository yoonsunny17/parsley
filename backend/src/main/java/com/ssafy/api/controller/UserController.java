...
public class UserController {
    ...
    @PostMapping("/rejoin")
    @ApiOperation(value = "회원 재가입", notes = "탈퇴한 회원을 재가입시킨다.")
    @ApiResponses({
            @ApiResponse(code = 201, message = "회원 재가입 성공"),
            @ApiResponse(code = 500, message = "서버 오류")
    })
    public ResponseEntity<? extends UserPostRes> rejoinUser(@RequestBody UserRejoinReq userInfo) {

        Long userId = userInfo.getUserId();

        User user = userService.getUserByUserId(userId);

        userService.rejoinUser(user);
